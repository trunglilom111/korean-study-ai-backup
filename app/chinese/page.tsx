...
import ChineseVocabularyLab from "@/components/ChineseVocabularyLab";
    ...
    <main className="min-h-screen overflow-x-hidden bg-[#080d1d] text-slate-100">
      <div className="relative isolate overflow-hidden border-b border-white/10">
        ...
        <header className="mx-auto flex w-full max-w-7xl items-center justify-between gap-4 px-5 py-5 md:px-10 md:py-7">
          ...
          <div className="flex items-center gap-2 text-sm"><a href="#lo-trinh" className="hidden rounded-xl px-3 py-2 text-slate-300 hover:text-white sm:block">Lộ trình</a><a href="#kho-tu" className="hidden rounded-xl px-3 py-2 text-slate-300 hover:text-white sm:block">Kho 75 từ</a><a href="#gemini" className="hidden rounded-xl px-3 py-2 text-slate-300 hover:text-white lg:block">Gemini AI</a><Link href="/" className="rounded-xl border border-slate-700 bg-slate-900/80 px-3 py-2 font-medium text-slate-200 transition hover:border-slate-500">KR · Tiếng Hàn</Link></div>
        </header>
        <section className="mx-auto grid max-w-7xl gap-10 px-5 pb-16 pt-8 md:px-10 lg:grid-cols-[1.1fr_0.9fr] lg:items-center lg:pb-24 lg:pt-14">
          <div><p className="inline-flex rounded-full border border-red-400/35 bg-red-950/40 px-3 py-1.5 text-xs font-semibold tracking-wide text-red-200">HỌC CÓ LỘ TRÌNH · ÔN CÓ HỆ THỐNG</p><h1 className="mt-5 max-w-3xl text-4xl font-black leading-[1.08] tracking-tight text-white md:text-6xl">Từ <span className="text-red-400">pinyin</span> đến mục tiêu HSK 3–4.</h1><p className="mt-5 max-w-2xl text-base leading-7 text-slate-300 md:text-lg">Bắt đầu từ âm, thanh điệu và câu ngắn. Mỗi chặng có mục tiêu, kho từ tăng dần và bài kiểm tra để bạn học được mỗi ngày.</p><div className="mt-8 flex flex-wrap gap-3"><a href="#kho-tu" className="rounded-xl bg-red-500 px-5 py-3 font-bold text-white shadow-lg shadow-red-950/40 transition hover:bg-red-400">Học kho 75 từ</a><a href="#chan-doan" className="rounded-xl border border-slate-600 bg-slate-900/70 px-5 py-3 font-semibold text-slate-200 transition hover:border-slate-400">Chọn điểm bắt đầu</a></div><div className="mt-8 flex flex-wrap gap-x-7 gap-y-3 text-sm text-slate-400"><span>✓ 75 từ theo 5 mức</span><span>✓ Flashcard + kiểm tra</span><span>✓ Lưu tiến độ trên máy</span></div></div>
